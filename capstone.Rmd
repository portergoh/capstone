...
cluster_df <- cluster_df_grp %>%
  select(avg_rent, x_coord, y_coord) %>%
...
train_set %>%
  select(median_rent,condo_age,mrt_dist) %>%
...
# An intercept that is different for each district and region but keeping fixed effects
# coefficient the same across all districts and regions
m2 <- lmer(log(median_rent) ~  mrt_dist_f + condo_age + mrt_dist_f:condo_age + 
            (1|district),
            REML=TRUE,
            data=train_set)

# An intercept that is different for each district but keeping interaction effect
# coefficient the same across all districts
m3 <- lmer(log(median_rent) ~ 1 + mrt_dist_f:condo_age + 
            (1|district), 
            REML=TRUE,
            data=train_set) 

# An intercept that is different for each region but keeping interaction effect
...
# Same intercept for each district but with varying age coefficients
m5 <- lmer(log(median_rent) ~ mrt_dist_f + condo_age + mrt_dist_f:condo_age +
            (0 + condo_age|district),
            REML=TRUE,
            data=train_set) 

# An intercept that is different for each district and different coefficient for age across all districts
