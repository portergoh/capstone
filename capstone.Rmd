---
title: "Analysis of Rental Rates of Private Non-landed Residential Properties in SG"
...
```{r, fig.height=8, fig.cap="Plot of Median Rental Rate/sqft vs Age of Condominium with glm4 model"}

ggplot(data=train_set) + 
  
  geom_point(aes(x=condo_age,y=median_rent), 
             position = 'jitter',
             alpha = 0.3, 
             size = 2) +
  
  geom_line(aes(x=condo_age,y=fitted(glm4),
                group=mrt_dist_f),
                color="tomato4",
                size=1) + 
...
# Varying slope for condo age in each district 
m3 <- lmer(median_rent ~ mrt_dist_f + condo_age + mrt_dist_f:condo_age +
...
m5 <- lmer(median_rent ~ mrt_dist_f + condo_age +
            (1|region),
            REML=TRUE,
            data=train_set) 
...
Our lollipop function provides a holistic way for users to view rental rate in each district with respect to the age of the property. This can act as a useful benchmark for bargain hunters or landlords when deciding to rent/buy a property in a particular district. It has the potential to develop further using Shiny into a web tool, to be used by property agents when marketing new properties to potential owners.
