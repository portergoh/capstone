...
# Removing incomplete cases which casue problem to clustering results

condo_dataset$region_l <-  factor(condo_dataset$region, 
                                  levels=c("CCR", "RCR", "OCR"))

condo_rent1c <- condo_dataset[complete.cases(condo_dataset),]

cluster_df_grp<- condo_rent1c %>%
  group_by(condo_name, district, region_l, x_coord, y_coord) %>%
...
Silhouette suggest optimal 5 cluster.
...
df_grp$range[df_grp$grp == 3] <- "High"
df_grp$range[df_grp$grp == 2] <- "Medium"
df_grp$range[df_grp$grp == 1] <- "Low"
