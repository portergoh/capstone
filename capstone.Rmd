...
p_load(devtools,bookdown,tidyverse,knitr,modelr,scatterplot3d,
       ...
       NbClust,factoextra,raster,rgdal,sp,proto,e1071,ranger,caret)
...
We could see that cluster 3 has the highest mean rental score follow by cluster 2 and 1.
This is very similar to our most expensive region CCR, follow by RCR and OCR.
...
We observed that within CCR, there are area of overlaps between our new clusters. And the interesting districts are 10 and 11 which happen to be in our new cluster 1 (least expensive) but in the most expensive CCR region.
...
<br>
Let's us take a closer look on district 10 and 11 using a lollipop chart.
...
<br>
Reviewing the geographical location, it appears that the highest and lowest rental condo are at opposite end of each District.

```{r echo=FALSE, message = FALSE, warning=FALSE,fig.width=10, fig.height=6}

# Where is District 10, 11

df_grp3 <- df_grp2 %>% filter(district == "10" |
                              district == "11")  
...
df_grp3 %>%
  ...
  geom_text(aes(label = condo_name, size = avg_rent, fontface = avg_rent),
            check_overlap = T) +
  guides(size = "none") +
...
We can see that the machine learning model AIC's score of 3702 matches quite closely with our m2 score of 3624. When fitted over new set of data, RMSE score is slightly better.
