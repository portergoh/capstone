...
We will explore the relationship of non-landed private residential properties rental rates using these 3 metrices to build our model.
...
Data for the private residential properties’ TOP (by year) and MRT proximity is obtained via web scrapping from the SingaporeExpats.com website and mytransport.sg:
...
p_load(devtools,bookdown,tidyverse,knitr,modelr,scatterplot3d,
       ...
       NbClust,factoextra,raster,rgdal,sp,proto)
...
# Create a dataframe with each condo mean value
cluster_df_grp <- train_set %>%
  group_by(condo_name, district, region, x_coord, y_coord) %>%
  summarise(avg_rent = mean(median_rent),
            avg_age = mean(condo_age),
            avg_mrt_dist = mean(mrt_dist)) 

cluster_df <- as_tibble(cluster_df_grp)
#glimpse(cluster_df_grp)
...
```{r, warning=FALSE,message=FALSE, fig.cap="Boxplot of Median Rental Rate/sqft vs Age of Condominium"}
...
train_set %>% 
  ...
  geom_boxplot(varwidth = TRUE, fill = "white", notch = FALSE) +
...
This plot seems to concur with the popular belief that if your property is located further away from MRT, it will become less attractive to users. However, some properties that are located more than 2 km away, appeared to have bucked the trend.
...
```{r, fig.cap="Boxplot of Median Rental Rate/sqft vs MRT distance"}
...
```{r, echo=TRUE, fig.cap="Median Rental Rates vs District Location of Condominium by Region"}

train_set %>%
  ggplot(aes(x = district, y = median_rent, fill = region)) + 
...
We can see that the all our scores improves drastically and anova test shows that this new model has more explanatory power. However one of the key assumption of anova is that our collected condo transaction should be independent of each other. 

From our domain knowledge, we know that the rental price is usually based on the recently transacted rental price of neighboring apartments, hence that would violate the assumption and therefore our results maybe bias.
...
The summary table below also concur with the earlier anova test results that glm3 is the best as it shows a lower score for AIC and BIC and higher Pseudo R2 when compared to the rest. We didnt include glm4 as it violate one of the key independent assumption.

**Notice that the R-Square score using jtools is slightly different from the rsq package we computed earlier.**

```{r, echo=FALSE}

export_summs(glm1,glm2,glm3,
             model.names = c("glm1","glm2","glm3"),
             digits=2,
...
In this model, we are allowing the intercepts to varying between each region while keeping the fixed effect coefficients constant.

```{r, echo=TRUE}

m1 <- lmer(log(median_rent) ~ mrt_dist_f * condo_age + 
               (1 | region),
               REML=TRUE,
               data=train_set)
...
<br>
The dark grey lines represent the best model we had when we used the generalized linear regression model. But we are still missing much of the variance explanation as we did not take into account the variation of condos between districts or regions.
...
# An intercept that is different for each district while keeping fixed effects 
# coefficients constant
m2 <- lmer(log(median_rent) ~  mrt_dist_f + condo_age + mrt_dist_f:condo_age + 
            (1|district),
            REML=TRUE,
            data=train_set)
...
# An intercept that is different for each district while keeping fixed effects 
# coefficients constant
m3 <- lmer(log(median_rent) ~ 1 + mrt_dist_f:condo_age + 
            (1|district), 
            REML=TRUE,
            data=train_set) 

# An intercept that is different for each region while keeping fixed effects 
# coefficients constant
m4 <- lmer(log(median_rent) ~ 1 + mrt_dist_f:condo_age + 
            (1|region),
            REML=TRUE,
            data=train_set) 

# Same intercept for each district but with varying age coefficients while keeping 
# fixed effects coefficients constant
m5 <- lmer(log(median_rent) ~ mrt_dist_f + condo_age + mrt_dist_f:condo_age +
            (0 + condo_age|district),
            REML=TRUE,
            data=train_set) 

# Same intercept for each region but with varying age coefficients while keeping 
# fixed effects coefficients constant
m6 <- lmer(log(median_rent) ~ mrt_dist_f + condo_age + mrt_dist_f:condo_age +
            (0 + condo_age|region),
            REML=TRUE,
            data=train_set) 

# An intercept that is different for each districts while keeping fixed 
# effects coefficients constant
m7 <- lmer(log(median_rent) ~ mrt_dist_f + condo_age + mrt_dist_f:condo_age + 
            (1 | district),
            REML=TRUE,
            data=train_set) 
...
### Is m6 better than m5 ?

Even though m6 has the best overall AIC/BIC but lower R2 score, adding random slopes for age across regions doesn't seem to significantly improve our model fit.

```{r, echo=TRUE, results="asis"}

kable(anova(m5,m6, refit=FALSE), digits=2)

```

### Summary of Results

The best model, according to AIC/BIC and R-square score is model m5 that assumes different fixed slopes for each level of mrt distance and random age coefficients across all districts.

```{r}

export_summs(m1,m2,m3,m4,m5,m6,m7,
             model.names = c("m1","m2","m3","m4","m5","m6","m7"),
             digits=2,
...
We have random age coefficients across all 28 districts.

```{r, echo=TRUE, results="asis"}

kable(coef(m5)$district, digits=2,
...
simulationOutput <- simulateResiduals(fittedModel=m5, n=250,refit=T,seed=123)
...
Let's use the test dataset we have obtained earlier and try to fit it using our best model m5
...
We can see that our test set has a lower RMSE value than our training set which shows that our model provide a comparatively good fit.

```{r, echo=TRUE}

m5_pred_train  <- exp(predict(m5, train_set))
m5_error_train <- m5_pred_train - train_set$median_rent
m5_RMSE_train  <- sqrt(mean(m5_error_train^2))
m5_RMSE_train
...
m5_pred_test  <- exp(predict(m5, test_set))
m5_error_test <- m5_pred_test - test_set$median_rent
m5_RMSE_test  <- sqrt(mean(m5_error_test^2))
m5_RMSE_test
...
We hope to use this clustering method to rediscover new groupings that may fit our model better.
...
# Scaling and Centering of numeric variables 
df <- cluster_df %>%
  dplyr::select(avg_rent, x_coord, y_coord) %>%
...
fviz_dend(res_ward2, k = 3, # Cut in 3 groups
          cex = 0.8, # label size
...
# Put cluster name to the original dataset
df_grp <- data.frame(cluster_df, grp)
#glimpse(df_grp)

# Profiling the clusters
# Statistic summary of Cluster Group
clus_profile <- data.frame (
  df_grp %>% group_by(Cluster = grp) %>%
  summarise(Freq = n(),
            mean_rent = mean(avg_rent),
            sd_rent = sd(avg_rent),
            mean_age = mean(avg_age),
            sd_age = sd(avg_age))
)

kable(clus_profile, digits=2)

df_grp$range[df_grp$grp == 1] <- "High"
df_grp$range[df_grp$grp == 2] <- "Medium"
df_grp$range[df_grp$grp == 3] <- "Low"
...
# Add mean and sd of median_rent for each district in each group
avg_table <- as_tibble(df_grp %>% 
                        group_by(grp, district) %>%
                        summarise(avg = round(mean(avg_rent),2),
                                  sd = round(sd(avg_rent),2)))
...
district_table <- as_tibble(df_grp %>% 
                            group_by(district) %>%
...
```{r, echo=TRUE, fig.height=8, fig.cap="Heap map plot with respect to cluster and region group"}

# Visualise Heat Map
df_grp2 %>% 
  ggplot(aes(x = district, y = range, fill = avg_rent)) +
  geom_tile(col = "white") + 
  ...
  labs(title = "Result of Hierarchical Clustering",
       subtitle = "Not all Condo in the same region are the same",
       x="District Number", 
...
# Looking into District 10, 11, 05, 12, 14, 21
# Observation: 
# Generally, District 10, 11, 05, 12, 14, 21 have mixed on condo with
# lower average rental rates. It is noted that the age on condo tend to be older

plot_Lollipop_district <- function(dist){
  ...
  summarise(avg_condo = mean(avg_rent),
            avg_condo_age = mean(avg_age))
  ...
  labs(title = paste("Listing of Condo in District ", dist, sep = ""),
...
```{r, echo=TRUE, fig.height=8, fig.width=10, fig.cap="3 Clustered Range of Median Rent Rates"}
...
df_grp2_p +
  ...
  labs(title = "Result of Hierarchical Clustering", 
       subtitle = "3 Clustered Range of Median Rent Rates",
       caption = "Source: Urban Redevelopment Authority (URA)",
       x="",
       y="") +
...
![](https://site.listsothebysrealty.sg/wp-content/uploads/2019/01/3-02.jpg)
